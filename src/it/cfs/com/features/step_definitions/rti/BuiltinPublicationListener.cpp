...
log4cxx::LoggerPtr BuiltinPublicationListener::logger = log4cxx::Logger::getLogger(std::string("cfs.com.features.step_definitions.rti.builtinpublicationlistener"));

void BuiltinPublicationListener::on_data_available(DataReader<PublicationBuiltinTopicData>& reader)
{
            ...
            sample_it++) 
    {
	    ...
	    LOG4CXX_DEBUG(logger , "participant: " << partKey.value()[0]
            ...
            if (!data->type().is_set()) 
            {
		LOG4CXX_TRACE(logger , "No type received, perhaps increase type_code_max_serialized_length?");
            } 
            else 
            {
